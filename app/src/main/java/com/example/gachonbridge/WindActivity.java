    ...
    private void loadWindData() {
        ...
        new Thread(() -> {
            try {
                ...
                android.util.Log.d("WIND_DEBUG", "전체 고유 프로그램 수: " + all.size());
                for (int i = 0; i < Math.min(5, top3.size()); i++) {
                    android.util.Log.d("WIND_DEBUG", (i+1) + "위: " + top3.get(i).hits + " | " + top3.get(i).title);
                }
                final List<WindProgram> top3Final = top3.subList(0, Math.min(3, top3.size()));
                android.util.Log.d("WIND_DEBUG", "top3Final 크기: " + top3Final.size());
