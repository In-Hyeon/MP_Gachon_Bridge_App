...
import android.content.Intent;import android.content.SharedPreferences;import android.graphics.Color;import android.graphics.Typeface;import android.graphics.drawable.GradientDrawable;import android.net.Uri;import android.os.Bundle;import android.view.Gravity;import android.view.View;import android.widget.Button;import android.widget.EditText;import android.widget.LinearLayout;import android.widget.TextView;import android.widget.Toast;
...
public class LabActivity extends BaseActivity {
    ...
    private Button btnTabAi;
    private Button btnTabBusiness;
    private Button btnTabChem;
    private Button btnSearchProfessor;
    private Button btnMoreLabs;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedFavorites != null) {
            favoriteProfessorNames = new HashSet<>(savedFavorites);
        }
    ...
    private void searchProfessor() {
        ...
        List<Lab> searchResult = new ArrayList<>();
        List<Lab> baseLabs = getLabsByFavoriteMode();

        for (Lab lab : baseLabs) {
            String normalizedProfessor = lab.professor.replace(" ", "");

            if (lab.professor.contains(keyword) || normalizedProfessor.contains(normalizedKeyword)) {
                searchResult.add(lab);
            }
        }

        isSearchMode = true;
        renderLabs(searchResult);
    ...
    private void showMoreLabs() {
        List<Lab> targetLabs = getLabsByFavoriteMode();

        if (targetLabs == null || targetLabs.isEmpty()) {
            return;
        }

        isSearchMode = false;
        visibleCount += PAGE_SIZE;
        renderLabs(targetLabs);
    }

    private List<Lab> getLabsByFavoriteMode() {
        if (!favoritesOnly) {
            return new ArrayList<>(currentLabs);
        }

        List<Lab> favoriteLabs = new ArrayList<>();

        for (Lab lab : currentLabs) {
            if (favoriteProfessorNames.contains(lab.professor)) {
                favoriteLabs.add(lab);
            }
        }
    ...
    private void saveFavorites() {
        sharedPreferences.edit()
                .putStringSet(PREF_FAVORITES, new HashSet<>(favoriteProfessorNames))
                .apply();
    }
    ...
    private void setFavoriteModeButtonStyle(Button button, boolean selected) {
        ...
        if (selected) {
            ...
            button.setBackground(makeRoundRectWithStroke("#24262E", "#3B3F50", 16, 1));
        }
    }
    ...
    private void setSelectedTab(Button selectedButton) {
        ...
        setTabStyle(selectedButton, true);
    }
    ...
    private void setTabStyle(Button button, boolean selected) {
        ...
        if (selected) {
            ...
            button.setTextColor(Color.WHITE);
            button.setBackground(makeRoundRectWithStroke("#24262E", "#3B3F50", 18, 1));
        }
    }
    ...
    private void renderLabs(List<Lab> labs) {
        ...
        if (labs == null || labs.isEmpty()) {
            ...
            if (favoritesOnly && !isSearchMode) {
                emptyMessage = "즐겨찾기한 연구실이 없습니다.";
            } else {
                emptyMessage = "검색 결과가 없습니다.";
            }

            TextView emptyView = makeText(emptyMessage, "#A8A8A8", 14, false);
            ...
            labListLayout.addView(emptyView, emptyParams);
            btnMoreLabs.setVisibility(View.GONE);
            return;
        }
        ...
        if (isSearchMode) {
            ...
            count = Math.min(visibleCount, labs.size());
        }

        for (int i = 0; i < count; i++) {
            labListLayout.addView(createLabCard(labs.get(i)));
        }

        if (!isSearchMode && visibleCount < labs.size()) {
            btnMoreLabs.setVisibility(View.VISIBLE);
        } else {
            btnMoreLabs.setVisibility(View.GONE);
        }
    }
    ...
    private View createLabCard(Lab lab) {
        ...
        card.setBackground(makeRoundRect("#FFFFFF", 4));
        ...
        TextView labNameView = makeText(lab.labName, "#3C4752", 17, true);
        infoBox.addView(labNameView);

        TextView professorView = makeText(
                lab.professor + " / " + lab.position,
                "#4A4A4A",
                13,
                true
        );
        professorView.setPadding(0, dp(6), 0, 0);
        infoBox.addView(professorView);

        TextView researchView = makeText(lab.research, "#5F666D", 13, false);
        researchView.setPadding(0, dp(6), 0, 0);
        infoBox.addView(researchView);

        topRow.addView(infoBox, infoParams);

        TextView favoriteStar = new TextView(this);

        if (favoriteProfessorNames.contains(lab.professor)) {
            favoriteStar.setText("★");
            favoriteStar.setTextColor(Color.parseColor("#F6C344"));
        } else {
            favoriteStar.setText("☆");
            favoriteStar.setTextColor(Color.parseColor("#9AA0AA"));
        }

        favoriteStar.setTextSize(28);
        favoriteStar.setGravity(Gravity.CENTER);
        favoriteStar.setTypeface(Typeface.DEFAULT, Typeface.BOLD);
        favoriteStar.setOnClickListener(v -> toggleFavorite(lab));

        LinearLayout.LayoutParams starParams = new LinearLayout.LayoutParams(dp(40), dp(40));
        starParams.setMargins(dp(6), 0, 0, 0);
        topRow.addView(favoriteStar, starParams);

        card.addView(topRow);

        View divider = new View(this);
        divider.setBackgroundColor(Color.parseColor("#E7E7E7"));
        ...
        detailButton.setTextColor(Color.parseColor("#111318"));
        detailButton.setAllCaps(false);
        detailButton.setMinWidth(0);
        detailButton.setMinHeight(0);
        detailButton.setBackground(makeRoundRect("#8EA8FF", 4));
        detailButton.setOnClickListener(v -> openHomepage(lab.homepage));

        LinearLayout.LayoutParams detailParams = new LinearLayout.LayoutParams(dp(105), dp(40));
        buttonRow.addView(detailButton, detailParams);
    ...
    private void addInfoRow(LinearLayout parent, String label, String value) {
        ...
        TextView labelView = makeText(label, "#4A4A4A", 13, true);
        LinearLayout.LayoutParams labelParams = new LinearLayout.LayoutParams(
                dp(72),
                LinearLayout.LayoutParams.WRAP_CONTENT
        );

        TextView valueView = makeText(value, "#5E6670", 13, false);
    ...
    private void openHomepage(String homepage) {
        ...
        try {
            ...
            Toast.makeText(this, "홈페이지를 열 수 없습니다.", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private List<Lab> getAiLabs() {
        return Arrays.asList(
                new Lab(
                        ...
                        "강상우",
                        "부교수",
                        "자연어처리, LLM, 대화형 QA 시스템",
                        "031-750-8669",
                        "swkang@gachon.ac.kr",
                        "https://sites.google.com/view/kangsangwoo",
                        "AI관 419호"
                ),
                new Lab(
                        "구",
                        ...
                        "계산 이미징",
                        "031-750-6967",
                        "jakeoung@gachon.ac.kr",
                        "https://sites.google.com/view/jakeoung",
                        "AI관 433호"
                ),
                new Lab(
                        "노",
                        ...
                        "노웅기",
                        "교수",
                        "대용량 데이터 마이닝, 유전자 데이터베이스",
                        "031-750-5349",
                        "wkloh2@gachon.ac.kr",
                        "http://professor.gachon.ac.kr/main/main.jsp?Forum_seq=88524",
                        "AI관 422호"
                ),
                new Lab(
                        "민",
                        ...
                        "031-750-5828",
                        "hmin@gachon.ac.kr",
                        "https://sites.google.com/view/hmin",
                        "AI관 431호"
                ),
                new Lab(
                        "안",
                        ...
                        "https://sites.google.com/view/vip-lab",
                        "AI관 740호"
                ),
                new Lab(
                        "엄",
                        ...
                        "엄광현",
                        "조교수",
                        "의료 인공지능, 의료 영상 분석, 정밀의료",
                        "031-750-8850",
                        "khuhm@gachon.ac.kr",
                        "https://sites.google.com/view/aimhi-lab",
                        "AI관 737호"
                ),
                new Lab(
                        "오",
                        ...
                        "오현영",
                        "조교수",
                        "시스템 보안, 하드웨어 기반 보안, AI 보안",
                        "031-750-5504",
                        "hyoh@gachon.ac.kr",
                        "https://sites.google.com/view/hyoh",
                        "AI관 741호"
                ),
                new Lab(
                        "유",
                        ...
                        "유준",
                        "교수",
                        "자율주행자동차 통신, 클라우드 데이터센터 통신",
                        "031-750-5832",
                        "joon.yoo@gachon.ac.kr",
                        "https://sites.google.com/view/winl",
                        "AI관 423호"
                ),
                new Lab(
                        "이",
                ...
                new Lab(
                        ...
                        "031-750-8658",
                        "yjung@gachon.ac.kr",
                        "https://sites.google.com/site/gachoncvip/",
                        "AI관 430호"
                ),
                new Lab(
                        "정",
                        ...
                        "정윤현",
                        "부교수",
                        "컴퓨터 그래픽스, 혼합현실, 머신러닝, 의료 영상",
                        "031-750-4771",
                        "younhyun.jung@gachon.ac.kr",
                        "https://gcu-vomlab.github.io/VOM/index.html",
                        "AI관 421호"
                ),
                new Lab(
                        "조",
                ...
                new Lab(
                        ...
                        "031-750-5353",
                        "pjcho@gachon.ac.kr",
                        "https://sites.google.com/view/fdslab",
                        "AI관 417호"
                ),
                new Lab(
                        "조",
                        ...
                        "조해민",
                        "조교수",
                        "로보틱스, 로봇 지능 제어, 자율주행 기술",
                        "031-750-5320",
                        "hmcho9@gachon.ac.kr",
                        "https://hxxmin.github.io/rai-lab/",
                        "AI관 736호"
                ),
                new Lab(
                        "최",
                        ...
                        "최아영",
                        "부교수",
                        "인간 컴퓨터 상호작용, 헬스케어, 감성인식",
                        "031-750-8656",
                        "aychoi@gachon.ac.kr",
                        "https://sites.google.com/site/ahyoungchoi00/home",
                        "AI관 434호"
                ),
                new Lab(
                        "최",
                        ...
                        "https://sites.google.com/view/jaeyoungchoi/home",
                        "AI관 420호"
                ),
                new Lab(
                        "최",
                        ...
                        "최재용",
                        "조교수",
                        "지능형 로봇, 무인 시스템, 멀티모달 AI",
                        "031-750-8667",
                        "andrewjchoi@gachon.ac.kr",
                        "https://sites.google.com/view/irasc",
                        "AI관 428호"
                ),
                new Lab(
                        "최",
    ...
    private List<Lab> getBusinessLabs() {
        return Arrays.asList(
                new Lab(
                        ...
                        "https://www.gachon.ac.kr/biz/4446/subview.do",
                        "가천관 806호"
                ),
                new Lab(
                        "곽",
                ...
                new Lab(
                        ...
                        "김산월",
                        "교수",
                        "생성형 AI, 디지털 전환, ESG, 지속가능성, 기업재무",
                        "031-750-5757",
                        "jsyrena0923@gachon.ac.kr",
                        "https://sites.google.com/gachon.ac.kr/jsy",
                        "가천관 609호"
                ),
                new Lab(
                        "김",
                ...
                new Lab(
                        ...
                        "김홍범",
                        "조교수",
                        "기술혁신, 운영관리, IT 전략, 산업정책",
                        "031-750-5176",
                        "hkim@gachon.ac.kr",
                        "https://sites.google.com/view/hongbumkim/home",
                        "가천관 707호"
                ),
                new Lab(
                        "김",
                ...
                new Lab(
                        ...
                        "엄금철",
                        "부교수",
                        "생성형 AI, Human-Machine Interaction, 텍스트마이닝, 추천시스템",
                        "031-750-5181",
                        "yanjz@gachon.ac.kr",
                        "https://sites.google.com/gachon.ac.kr/yanjz/home",
                        "가천관 711호"
                ),
                new Lab(
                        "왕",
                        ...
                        "왕맹맹",
                        "조교수",
                        "빅데이터 분석, 디지털 비즈니스 전환, e-Business 전략",
                        "031-750-8744",
                        "bizmwang@gachon.ac.kr",
                        "https://sites.google.com/view/bizmwang",
                        "중앙도서관 432호"
                )
        );
    }
    ...
    private List<Lab> getChemLabs() {
        return Arrays.asList(
                new Lab(
                        ...
                        "031-750-5409",
                        "학과 페이지 참조",
                        "https://www.gachon.ac.kr/chemistry/6378/subview.do",
                        "반도체대학 6-7호"
                ),
                new Lab(
                        "김",
                        ...
                        "김명종",
                        "교수",
                        "나노사이언스, BNNT, 에너지 저장/촉매 소재",
                        "031-750-8721",
                        "myungjongkim@gachon.ac.kr",
                        "https://snclweb.github.io/gachon/index.html",
                        "바이오나노연구원 514호"
                ),
                new Lab(
                        "김",
                        ...
                        "김연호",
                        "부교수",
                        "광촉매, CO2 전환, 차세대 이차전지 전극 소재",
                        "031-750-8558",
                        "yeonho@gachon.ac.kr",
                        "https://sites.google.com/view/acemlab",
                        "예술체육대학1 4-10"
                ),
                new Lab(
                        "김",
                ...
                new Lab(
                        ...
                        "송하영",
                        "조교수",
                        "합성 무기화학, 유기금속, 초분자, 생무기화학",
                        "031-750-5410",
                        "학과 페이지 참조",
                        "https://sites.google.com/view/hayoungsong/",
                        "바이오나노연구원 211호"
                ),
                new Lab(
                        "안",
                        ...
                        "화학생물학, 바이오소재",
                        "031-750-8559",
                        "suns2000@gachon.ac.kr",
                        "https://sites.google.com/view/anlab/",
                        "예술체육대학1 3-9"
                ),
                new Lab(
                        "이",
                ...
                new Lab(
                        ...
                        "031-750-8826",
                        "clee@gachon.ac.kr",
                        "https://sites.google.com/view/cwleegroup/",
                        "반도체대학 5-9"
                ),
                new Lab(
                        "장",
    ...
    private GradientDrawable makeRoundRect(String color, int radiusDp) {
    ...
    private GradientDrawable makeRoundRectWithStroke(
            String color,
            String strokeColor,
            int radiusDp,
            int strokeWidthDp
